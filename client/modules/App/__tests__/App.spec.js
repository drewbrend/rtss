...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Helmet').length, 0);
  t.is(wrapper.find('Header').length, 1);
  t.is(wrapper.find('Footer').length, 1);
  t.is(wrapper.find('Header').prop('toggleAddTest'), wrapper.instance().toggleAddTestSection);
...
test('calling toggleAddTestSection dispatches toggleAddTest', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddTestSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddTest()));
});
